...
const SubflowSuggestions = (props) => {
	...
	const executeTestWorkflow = (workflowid) => {
		const data = { "execution_argument": '{"title":"THIS IS TEST ALERT","description":"TEST ALERT FROM NEXUSGUARD","reference_url": "nexusguardr.io"}' }
	...
	const generateEmailNotificationWorkflow = async (appname,appImage,nexusguardAPIKey,recepients) => {
	//currently only supports  figure out a way to support more apps
		const workflowName = `[GENERATED] ${appname} notification workflow`
		const workflowDescription = "Generated by NexusGuard for sending info/error notifications."
		const data = {
			"name": workflowName,
				...
				if (appname.toLowerCase() === "email"){
					...
					var workflowBody = {
						...
						"actions": [
							{
								"app_name": "email",
								"name": "send_email_nexusguard",
								"large_image":appImage,
								"isStartNode": true,
								"label": "send_email_nexusguard",
								"app_version": "1.3.0",
								"parameters": [
									{
										"name": "apikey",
										"value": nexusguardAPIKey
									},
									{
										"name": "recipients",
										"value": recepients
					...
					<>
						...
						<Box mt={2}>
							...
							<TextField
								...
								margin="normal"
								variant="outlined"
								placeholder={ selectedAppDetails.name.toLowerCase() === "jira" ? "Project key" : "NexusGuard API key"}
